static char *msgmng_id = 
	"@(#)Copyright (C) H.Shirouzu 1996-2016   msgmng.cpp	Ver4.00";
/* ========================================================================
...
ULONG MsgMng::MakeMsg(char *buf, int _packetNo, ULONG command, const char *msg, const char *exMsg, int *packet_len)
{
	...
	if (is_br_cmd) {
		...
		pkt_len += sprintf(buf + pkt_len, "\nUN:%s\nHN:%s\nNN:%s\nGN:%s\nVS:%s\nES:%x",
						local.userName, local.hostName, msg_org, exMsg_org, GetVerHexInfo(),
						exStatus);
	}
...
BOOL MsgMng::MakeEncryptMsg(PubKey *pubkey, ULONG packet_no, char *msgstr, bool is_extenc,
	char *share, char *ulist_str, char *buf)
{
	...
	int			sharelen = 0;
	int			ulistlen = 0;
	int			capa = pubkey->Capa() & GetLocalCapa(cfg);
		...
		if (ulist && ulist[0] == '\n' && (msg->command & IPMSG_CAPUTF8OPT)) {
			for (tok=separate_token(ulist, '\n', &p); tok; tok=separate_token(NULL, '\n', &p)) {
				if      (strncmp(tok, "UN:", 3) == 0) {
					...
					case IPMSG_BR_ENTRY: case IPMSG_BR_NOTIFY:
				...
				else if (strncmp(tok, "ES:", 3) == 0) {
					msg->exStatus = strtoul(tok+3, 0, 16);
				}
			}
		}
	}
...
BOOL MsgMng::DecryptMsg(MsgBuf *_msg, UINT *_cryptCapa, UINT *_logOpt)
{
	...
	char		*capa_hex, *skey_hex, *msg_hex, *hash_hex=NULL, *p;
	...
	if (cryptCapa & IPMSG_AES_256) {	// AES
		...
		*(ALG_ID *)(skey + 8) = CALG_RSA_KEYX;
		...
		if (!::CryptImportKey(target_csp, skey, len + SKEY_HEADER_SIZE, hExKey, 0, &hKey))
			return	sprintf(msg.msgBuf, "CryptImportKey Err(%X)", GetLastError()), FALSE;
...
#define PUB_KEYLEN		(2048 / 8)
#define SHARE_KEYLEN	(256  / 8)

int EncryptDataSize(Cfg *cfg, PubKey *pubKey, int in_len)
{
	return	PUB_KEYLEN + ALIGN_SIZE(in_len+1, AES_BLOCK_SIZE);
}

int EncryptData(Cfg *cfg, PubKey *pubKey, const BYTE *in, int in_len,
	BYTE *out, int max_len)
{
	...
	// KeyBlob 作成
	pubKey->KeyBlob(key, sizeof(key), &key_len);

	// 送信先公開鍵の import
	if (!::CryptImportKey(csp, key, key_len, 0, 0, &hExKey)) {
		Debug("CryptImportKey err(%x)\n", GetLastError());
		return	0;
	}

	// AES 用ランダム鍵作成
	key_len = SHARE_KEYLEN;
	if (!::CryptGenRandom(csp, key_len, key)) {
	...
	if (!::CryptEncrypt(hExKey, 0, TRUE, 0, out, (DWORD *)&key_len, sizeof(key))) {
	...
	if (in_len + key_len >= max_len) {
		return	0;
	}

	int		len = aes.EncryptCBC(in, out + key_len, in_len);
...
int DecryptData(Cfg *cfg, const BYTE *in, int in_len, BYTE *out, int max_len)
{
	HCRYPTPROV	csp		= cfg->priv[KEY_2048].hCsp;
	HCRYPTKEY	hExKey	= cfg->priv[KEY_2048].hKey;
	int			key_len = PUB_KEYLEN;
	BYTE		key[MAX_BUF] = {};
	...
	// AES 用共通鍵のセット
	AES		aes(key, key_len);
	int		len = aes.DecryptCBC(in + PUB_KEYLEN, out, in_len);
...
void MsgMng::UListToHostSub(const char *ulist, std::vector<HostSub> *hosts)
{
	hosts->clear();
	if (!ulist || !*ulist || strncmp(ulist, TOLIST_KEY, TOLIST_KEYSIZE) != 0) {
		return;
	}
		...
		for ( ; tok; tok=sep_tok(NULL, HOSTLIST_SEPARATOR, &p)) {
			...
			if (char *uname = sep_tok(tok, ':', &p2)) {
				if (char *hname = sep_tok(NULL, ':', &p2)) {
					...
					if (IsSameHost(&hostSub, &local)) {
						// remove self
						//hosts->insert(hosts->begin(), hostSub);
					}
					else {
...
void MsgMng::HostSubToUList(const std::vector<HostSub> &hosts, char *ulist)
{
	if (hosts.size() <= 0) {
		*ulist = 0;
		return;
